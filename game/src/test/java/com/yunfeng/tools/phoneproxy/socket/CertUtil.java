...
//	}
	 ...
	 * 从文件加载RSA公钥 openssl rsa -in ca.key -pubout -outform DER -out ca_pub.der
	 */
	public static PublicKey loadPubKey(InputStream inputStream)
			throws Exception {
		ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
		byte[] bts = new byte[1024];
		int len;
	...
	public static String getSubject(InputStream inputStream) throws Exception {
		...
		// 读出来顺序是反的需要反转下
		// List<String> tempList = Arrays.asList(certificate.getIssuerDN()
		// .toString().split(", "));
		// IntStream rangeClosed = IntStream.rangeClosed(0, tempList.size() -
		// 1);
		// Stream<String> str = rangeClosed.mapToObj(new IntFunction<String>() {
		//
		// @Override
		// public String apply(int value) {
		// tempList.get(tempList.size() - value - 1)).collect(
		// Collectors.joining(", "));
		// return null;
		// }
		// });
		// String str = rangeClosed.mapToObj(
		// i -> tempList.get(tempList.size() - i - 1)).collect(
		// Collectors.joining(", "));
		// Log.d(str);
	...
	public static X509Certificate genCert(String issuer,
		...
		String subject = "C=cn, ST=sz, L=js, O=ssl, OU=ssl, CN=xll" + hosts[0];
		// doc from https://www.cryptoworkshop.com/guide/
		JcaX509v3CertificateBuilder jv3Builder = new JcaX509v3CertificateBuilder(
				new X500Name(issuer), BigInteger.ONE, new Date(), new Date(
						System.currentTimeMillis() + TEN_YEAR), new X500Name(
						subject), serverPubKey);
		...
		jv3Builder.addExtension(Extension.subjectAlternativeName, false,
				subjectAltName);
		// SHA256 用SHA1浏览器可能会提示证书不安全
		ContentSigner signer = new JcaContentSignerBuilder(
				"SHA256WithRSA").build(caPriKey);
		return new JcaX509CertificateConverter().getCertificate(jv3Builder
				.build(signer));
	}
